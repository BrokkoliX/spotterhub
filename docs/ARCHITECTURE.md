...
> Generated from codebase analysis. Last updated: 2026-05-22.
...
erDiagram
  ...
  Photo {
    ...
    string watermarkPosition "enum: WatermarkPosition, nullable"
    int watermarkSize "percent of image width, 2-10, nullable"
    int watermarkOpacity "percent, 30-100, nullable"
...
| **Photo**                    | id, userId, albumId, caption, airline, airportCode, takenAt, originalUrl, originalWidth/Height, fileSizeBytes, mimeType, moderationStatus, moderationLabels, likeCount, hasActiveListing, license, watermarkEnabled, watermarkPosition, watermarkSize, watermarkOpacity, aircraftId, photographerId, gearBody, gearLens, exifData (JSON), photoCategoryId, aircraftSpecificCategoryId, operatorIcao, operatorType, msn, manufacturingDate, isDeleted | Core content; soft-delete via isDeleted                                             |
...
| `/map`                                                                                                                                                                                                                                              | PHOTOS_IN_BOUNDS                                                                                                                         | Photo markers only (no airport markers)                            |
| `/admin`                                                                                                                                                                                                                                            | ADMIN_STATS                                                                                                                              | —                                                                  |
...
**Counts are filtered** — `userPhotoCount`, airport `photoCount`, and album `photoCount` only include photos where `isDeleted: false` AND `moderationStatus: 'approved'`.

### 7.3 Image Variants

After S3 upload, `generateVariants()` creates: `thumbnail` (160px), `thumbnail_16x9` (640px wide), `display` (1920px), `full_res`, `watermarked`.

The `watermarked` variant is generated only when the uploader opts in. The watermark text is fixed at `© SpotterSpace` per `UploadRules.tsx` policy, but the uploader controls placement, size, and opacity through three persisted columns on `Photo`: `watermarkPosition` (one of nine `WatermarkPosition` enum values, e.g. `BOTTOM_RIGHT`), `watermarkSize` (integer 2-10, percent of image width), and `watermarkOpacity` (integer 30-100, percent). Defaults and bounds live in `WATERMARK_DEFAULTS` and `WATERMARK_BOUNDS` in `@spotterspace/shared`. The legacy `watermarkEnabled` boolean is now derived server-side from `watermarkConfig != null` and retained for backwards compatibility with existing rows. The GraphQL surface exposes a nullable `Photo.watermarkConfig: WatermarkConfig` object and accepts `CreatePhotoInput.watermarkConfig: WatermarkConfigInput`; validation is centralised in `clampWatermarkConfig()` which rejects out-of-range values with `BAD_USER_INPUT`.
...
### 7.6 AWS Infrastructure (as of 2026-05-22)

- **RDS**: `spotterhub-db` (db.t3.medium, 20GB gp3) + `wordmaster-db` (db.t3.micro, stopped)
- **ECS**: `spotterspace-cluster` — 1 Fargate task each for api and web. Dev task sizing: api 512 CPU / 1024 MB (task def :234), web 256 CPU / 512 MB (task def :181). Prod task sizing: api and web 1024 CPU / 2048 MB.
- **ALB**: `spotterspace-alb` (public)
- **VPC**: Main VPC (`vpc-09a6870488b73260e`) + unused VPC (`vpc-0a890a99f9551ecb8`)
- **S3 bucket**: `spotterhub-photos` (the CDK default was corrected from the non-existent `spotterspace-photos` on 2026-05-22)
- **Image tag strategy**: The CDK reads `API_IMAGE_TAG` and `WEB_IMAGE_TAG` at deploy time, defaulting to `latest`. Always supply an explicit git SHA when deploying to any environment.

The standard CDK deploy command is:
