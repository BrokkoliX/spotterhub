...
export default function AdminFamiliesPage() {
  ...
  const [formData, setFormData] = useState({ name: '', manufacturerId: '' });
  ...
  const [, upsertFamily] = useMutation(UPSERT_FAMILY);

  const families = data?.aircraftFamilies;
  const hasNextPage = families?.pageInfo?.hasNextPage;
  const endCursor = families?.pageInfo?.endCursor;
  const allManufacturers = manufacturersResult.data?.aircraftManufacturers?.edges?.map((e: { node: { id: string; name: string } }) => e.node) ?? [];
  // Build a name→id map for import lookups
  ...
  useEffect(() => {
    const rows = exportResult.data?.aircraftFamilies?.edges?.map((e: { node: FamilyNode }) => e.node);
    if (!rows || rows.length === 0) return;
    const csv = toCSVExport(rows);
    downloadCSV(csv, 'families.csv');
  ...
  const processImport = async () => {
    ...
    for (const row of pending) {
      ...
      if (!row.manufacturer_name) {
        done.push({ ...row, status: 'error' as const, message: 'Manufacturer name is required' });
        setImportRows([...done]);
        continue;
      }

      const manufacturerId = manufacturerNameToId.get(row.manufacturer_name.toLowerCase());
      if (!manufacturerId) {
        done.push({ ...row, status: 'error' as const, message: `Manufacturer "${row.manufacturer_name}" not found` });
        setImportRows([...done]);
        continue;
      }

      const res = await upsertFamily({ input: { name: row.name, manufacturerId } });
      const ok = !res.error;
  ...
  const openCreate = () => {
    setEditingId(null);
    setFormData({ name: '', manufacturerId: allManufacturers[0]?.id ?? '' });
    setFormError(null);
    setShowForm(true);
  };

  const openEdit = (node: FamilyNode) => {
    setEditingId(node.id);
    setFormData({ name: node.name, manufacturerId: node.manufacturer.id });
    setFormError(null);
    setShowForm(true);
  };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!input.manufacturerId) { setFormError('Manufacturer is required'); setFormLoading(false); return; }

    const result = editingId
      ? await updateFamily({ id: editingId, input })
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Delete this family? It will fail if any variants reference it.')) return;
  ...
  return (
    <div className={styles.page}>
      ...
      <h1 className={styles.title}>Aircraft Families</h1>
      ...
      <div className={styles.filters}>
        ...
        {families && (
          <span style={{ fontSize: '0.8125rem', color: 'var(--color-text-muted)' }}>
            {families.totalCount} family{families.totalCount !== 1 ? 'ies' : 'y'}
          </span>
        )}
      </div>

      {fetching && <div className={styles.loading}>Loading…</div>}
      {error && <div className={styles.loading}>Error loading families</div>}

      {families && families.edges.length > 0 && (
        <>
          <table className={styles.table}>
            ...
            <tbody>
              {families.edges.map(({ node }: { node: FamilyNode }) => (
                <tr key={node.id}>
                  <td>{node.name}</td>
      ...
      {families && families.edges.length === 0 && !fetching && (
        <div className={styles.loading}>No families found</div>
      )}
      </div>
      ...
      {showForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: 'var(--color-bg-card)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-lg)', padding: 24, width: '100%', maxWidth: 480 }}>
            <h2 style={{ marginBottom: 16, fontSize: '1.125rem' }}>{editingId ? 'Edit Family' : 'Add Family'}</h2>
            <form onSubmit={handleSubmit}>
              <div style={{ marginBottom: 12 }}>
                <label style={{ fontSize: '0.8125rem', display: 'block', marginBottom: 4 }}>Name *</label>
                <input className="input" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} required placeholder="737" style={{ width: '100%' }} />
              </div>
              ...
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                ...
                <button type="submit" className="btn btn-primary" disabled={formLoading}>{formLoading ? 'Saving…' : editingId ? 'Save Changes' : 'Add Family'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showImport && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: 'var(--color-bg-card)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-lg)', padding: 24, width: '100%', maxWidth: 720, maxHeight: '90vh', overflow: 'auto' }}>
            <h2 style={{ marginBottom: 8, fontSize: '1.125rem' }}>Import Families</h2>
            ...
            <div style={{ overflow: 'auto', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-sm)' }}>
              <table className={styles.table} style={{ margin: 0 }}>
                <thead>
                  <tr><th>Status</th><th>Name</th><th>Manufacturer</th></tr>
                </thead>
                <tbody>
...
function toCSVExport(rows: FamilyNode[]): string {
  ...
  const body = rows.map((r) => {
    ...
    return [esc(r.name), esc(r.manufacturer?.name)].join(',');
  });
