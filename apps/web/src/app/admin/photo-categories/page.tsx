...
import {
  ...
  CREATE_PHOTO_CATEGORY,
  DELETE_PHOTO_CATEGORY,
  EXPORT_PHOTO_CATEGORIES,
  UPDATE_PHOTO_CATEGORY,
  UPSERT_PHOTO_CATEGORY,
} from '@/lib/queries';
...
export default function AdminPhotoCategoriesPage() {
  ...
  const [exportResult, reexecuteExport] = useQuery({
    query: EXPORT_PHOTO_CATEGORIES,
    pause: true,
  });

  const [, createPhotoCategory] = useMutation(CREATE_PHOTO_CATEGORY);
  const [, updatePhotoCategory] = useMutation(UPDATE_PHOTO_CATEGORY);
  const [, deletePhotoCategory] = useMutation(DELETE_PHOTO_CATEGORY);
  const [, upsertPhotoCategory] = useMutation(UPSERT_PHOTO_CATEGORY);

  const categories = data?.photoCategories ?? [];
  ...
  useEffect(() => {
    const rows = exportResult.data?.photoCategories;
    if (!rows || rows.length === 0) return;
    const csv = toCSVExport(rows);
    downloadCSV(csv, 'photo-categories.csv');
  ...
  const processImport = async () => {
    ...
    for (const row of pending) {
      ...
      const res = await upsertPhotoCategory({ input });
      const ok = !res.error;
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const result = editingId
      ? await updatePhotoCategory({ id: editingId, input })
      : await createPhotoCategory({ input });
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Delete this category? Photos using it will lose this association.')) return;
  ...
  return (
    <div className={styles.page}>
      ...
      <h1 className={styles.title}>Photo Categories</h1>
      ...
      {categories.length > 0 && (
        <table className={styles.table}>
          <thead>
            <tr><th>Name</th><th>Label</th><th>Sort Order</th><th>Actions</th></tr>
          </thead>
          <tbody>
      ...
      {categories.length === 0 && !fetching && (
        <div className={styles.loading}>No photo categories found</div>
      )}
      </div>
      ...
      {showForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: 'var(--color-bg-card)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-lg)', padding: 24, width: '100%', maxWidth: 480 }}>
            <h2 style={{ marginBottom: 16, fontSize: '1.125rem' }}>{editingId ? 'Edit Photo Category' : 'Add Photo Category'}</h2>
            <form onSubmit={handleSubmit}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
                <div>
                  <label style={{ fontSize: '0.8125rem', display: 'block', marginBottom: 4 }}>Name *</label>
                  <input className="input" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} required placeholder="exterior" style={{ width: '100%' }} />
                </div>
                <div>
                  <label style={{ fontSize: '0.8125rem', display: 'block', marginBottom: 4 }}>Label *</label>
                  <input className="input" value={formData.label} onChange={(e) => setFormData({ ...formData, label: e.target.value })} required placeholder="Exterior" style={{ width: '100%' }} />
                </div>
              </div>
      ...
      {showImport && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: 'var(--color-bg-card)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-lg)', padding: 24, width: '100%', maxWidth: 720, maxHeight: '90vh', overflow: 'auto' }}>
            <h2 style={{ marginBottom: 8, fontSize: '1.125rem' }}>Import Photo Categories</h2>
